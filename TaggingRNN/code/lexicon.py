...
def problex_lexicon(corpus: TaggedCorpus) -> torch.Tensor:
    ...
    # Initialize counts
    word_tag_counts = torch.zeros((vocab_size, tagset_size))
    word_counts = torch.zeros(vocab_size)
    tag_counts = torch.zeros(tagset_size)
    ...
    # Add-one smoothing
    word_tag_counts += 1
    word_counts += tagset_size  # Add |T| for smoothing in denominator
    total_words = word_counts.sum().item()
    vocab_size_with_smoothing = vocab_size + 1

    # Compute p(t|w) and log(p(t|w))
    prob_tag_given_word = word_tag_counts / word_counts.unsqueeze(1)
    log_prob_tag_given_word = prob_tag_given_word.log()

    # Compute p(w) and log(p(w))
    prob_word = (word_counts + 1) / (total_words + vocab_size_with_smoothing)
    log_prob_word = prob_word.log()

    # Combine features into a matrix
    features = torch.cat((log_prob_tag_given_word, log_prob_word.unsqueeze(1)), dim=1)
    return features
